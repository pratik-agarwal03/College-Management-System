...
class UpdateTeacher implements ActionListener{
	...
	lab2.setFont(new Font("serif",Font.BOLD,20));
        ...
        dep = new JComboBox(branch);
        dep.setBackground(Color.WHITE);
        dep.setBounds(210,470,150,30);
        f.add(dep);
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==b){
            ...
            try{
                conn con = new conn();
                String str = "update teacher set name='"+t1.getText()+"',fathers_name='"+t2.getText()+"',last_name='"+t3.getText()+"', dob='"+t4.getText()+"',address='"+t5.getText()+"',phone='"+t6.getText()+"',email='"+t7.getText()+"',class_x='"+t8.getText()+"',class_xii='"+t9.getText()+"',aadhar='"+t10.getText()+"', emp_id = '"+empId+"',course='"+ed+"',dept='"+de+"' where emp_id='"+empId+"'";
        ...
        if(ae.getSource() == b2){
            try{
                conn con = new conn();
                String str = "select * from teacher where emp_id = '"+c2.getSelectedItem()+"'";
