...
class UpdateStudent implements ActionListener{
    ...
    UpdateStudent(){;
        ...
        c2.setBounds(200, 100, 200, 30);
        f.add(c2);
        
        
        b2 = new JButton("Check Details");
        b2.setBackground(Color.black);
        b2.setForeground(Color.white);
        b2.setBounds(420, 100, 200, 30);
        f.add(b2);
	...
	lab.setFont(new Font("serif",Font.BOLD,20));
        f.add(lab);
            //600,420,150,30
        String course[] = {"B.Tech","M.Tech","B.E","M.E"};
        co = new JComboBox(course);
        co.setBackground(Color.WHITE);
        co.setBounds(600,420,150,30);
        f.add(co);
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==b){
            String roll = c2.getSelectedItem();
            try{
                String cours = (String)co.getSelectedItem();
                String bran = (String)br.getSelectedItem();
                conn con = new conn();
                String str = "update student set name='"+t1.getText()+"',fathers_name='"+t2.getText()+"',last_name='"+t3.getText()+"', dob='"+t4.getText()+"',address='"+t5.getText()+"',phone='"+t6.getText()+"',email='"+t7.getText()+"',class_x='"+t8.getText()+"',class_xii='"+t9.getText()+"',aadhar='"+t10.getText()+"',course='"+cours+"',branch='"+bran+"' where rollno='"+roll+"'";
                con.s.executeUpdate(str);
                JOptionPane.showMessageDialog(null,"successfully updated");
                f.setVisible(false);
                new StudentDetailsComputer();
            }catch(Exception e){
        ...
        if(ae.getSource() == b2){
            try{
                conn con = new conn();
                String str = "select * from student where rollno = '"+c2.getSelectedItem()+"'";
